  ...
  el.innerHTML = `<tr><th>Flavor</th><th>Default Price</th><th>Your Price ($/bag)</th></tr>` +
    KQ.defaultFlavors.map(f => `
      <tr>
        ...
        <td><input type="number" min="0" step="0.50" placeholder="Enter price" value="${school&&school.prices&&school.prices[f.id]!=null?school.prices[f.id]:''}" data-fid="${f.id}" style="width:80px;padding:5px 8px;font-size:13px"></td>
      </tr>`).join('');
}
